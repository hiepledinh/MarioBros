...
namespace MarioBros
{
    ...
    abstract class MenuScreen : GameScreen
    {
        ...
        public MenuScreen(string menuTitle, ContentManager content, int padding)
        {
            // menus generally only need Tap for menu selection
        ...
        protected virtual void UpdateMenuEntryLocations()
        {
            ...
            Vector2 position = new Vector2(0f, ScreenManager.GraphicsDevice.Viewport.Height / 3);
        ...
        {
            float pos = ScreenManager.GraphicsDevice.Viewport.Width / 2 - MenuEntries[0].GetWidth(this) / 2;
            // make sure our entries are in the right place before we draw them
            if (!check)
            {
                UpdateMenuEntryLocations();
                if (MenuEntries[0].PositionX == pos)
                    check = true;
            }
